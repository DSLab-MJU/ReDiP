...
class TestDynamicObsAvoidance(Node):
    ...
    def __init__(self):
        ...
        self.wait_time = 20.0
        self.load_count = self.wait_time
        self.costmap_publish_timer = self.create_timer(self.wait_time, self.publish_global_costmap)
        self.navigation_system_load_timer = self.create_timer(1.0, self.print_log_remain_time)
        ...
        # self.api_request_url = "http://220.65.213.69:8000/inference-path/"
        self.action_horizon = 5

        self.semantic_map_input_dim = self.action_horizon # 이미지 입력 수는 10개로 지정.
    def print_log_remain_time(self):
        self.load_count -= 1
        
        if self.load_count == 0:
            self.get_logger().info(f"시스템 로드 완료%")    
            self.navigation_system_load_timer.cancel()

        self.get_logger().info(f"시스템 로드 중 {100-self.load_count}%")
    ...
    def inference_path(self, nagent_poses, nimages, ngoal):
        ...
        # Convert robot_poses and goal_poses to JSON strings
        # numpy 배열을 파이썬 리스트로 변환 후 직렬화
        robot_poses_str = json.dumps(nagent_poses.tolist()) if isinstance(nagent_poses, np.ndarray) else json.dumps(nagent_poses)
        goal_poses_str = json.dumps(ngoal.tolist()) if isinstance(ngoal, np.ndarray) else json.dumps(ngoal)
    ...
    def goal_callback(self, msg):
        ...
        if not isinstance(msg, PoseStamped):
            ...
            self.goal_pose = msg
        result = self.go_to_pose(self.goal_pose)
    ...
    def navigate_to_goal_recursive(self):
        ...
        if self.check_special_condition():
            ...
            if self.calc_goal_distance(self.robot_pose, self.goal_pose) > 0.45:
                ...
                return self.go_to_pose(self.goal_pose)
            else:
        ...
        time.sleep(0.1)
        self.get_logger().info(f"{navigation_start_status}")
