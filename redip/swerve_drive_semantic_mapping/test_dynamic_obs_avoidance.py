...
class TestDynamicObsAvoidance(Node):
    ...
    def __init__(self):
        ...
        self.path_publisher = self.create_publisher(Path, 'visualized_path', 10)
        ...
        self.map_data = None
        self.the_first_start_driving = True
    ...
    def initialize_diffuison_model(self):
        ...
        model_save_path = '/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/trained_finalmodel_datav13oa_policy_goalcond_20seq5img_t50_ep100.pth'
       # parameters
    ...
    def get_path_callback(self):
        if self.goal_pose:
            ...
            if self.initial_path is not None:
                ...
                path_msg.poses = self.initial_path.poses  # 경로의 포즈 목록을 Path 메시지에 추가
                
                # Path 토픽 발행
                self.path_publisher.publish(path_msg)
                self.get_logger().info("Path visualized and published.")
    ...
    #         # Extract metadata from the original costmap
    #         self.width = self.costmap.metadata.size_x
    #         self.height = self.costmap.metadata.size_y
    #         self.resolution = self.costmap.metadata.resolution
    #         self.origin_x = self.costmap.metadata.origin.position.x
    ...
    #         # Set padding size (in meters), e.g., extend by 1 meter in all directions
    #         padding_meters = 3.0
    #         padding_cells = int(padding_meters / self.resolution)  # Convert meters to cells (pixels)

    #         # Apply padding to the costmap image (expand the borders)
    #         self.costmap_np = self.initialize_map_image(self.costmap)
    #         self.costmap_np = self.reduce_inflation(self.costmap_np)

    #         # Add white padding around the costmap (fill value 255 for white)
    #         padded_costmap = cv2.copyMakeBorder(
    #             self.costmap_np,
    ...
    #         # Save the expanded costmap as an image
    #         cv2.imwrite("/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/map_data/cost_map.png", padded_costmap)

    #         # Update the global variables to reflect the new padded map size
    #         self.width = padded_costmap.shape[1]  # New width after padding
    #         self.height = padded_costmap.shape[0]  # New height after padding

    #         # Adjust the origin to account for the padding (move it to the bottom-left corner of the expanded map)
    #         self.origin_x -= padding_cells * self.resolution
    #         self.origin_y -= padding_cells * self.resolution
    ...
    #         # Update crop size based on the new resolution and area
    #         self.crop_size = int(self.area / self.resolution)
    #         self.half_crop_size = self.crop_size // 2

    #         # Initialize the cropped map and publish it
    #         global cost_map_image
    #         cost_map_image = padded_costmap

    #         # Adjusted cropped map for other purposes
    #         global map_image
    #         map_image = self.reduce_inflation(padded_costmap, 40)

    #         # Cancel the timer and set the crop center position
    #         self.costmap_publish_timer.cancel()
    #         # _, _ = self.set_crop_center_position(self.robot_pose)
    ...
    def trigger_custom_plan(self):
        ...
        if self.first_inference:
            ...
            self.get_logger().info(f"기존 네비게이션 작업을 취소합니다. 생성 전")
        # Custom 경로 생성
        ...
        if not self.first_inference:
            # 기존 네비게이션 작업을 취소
            self.get_logger().info(f"기존 네비게이션 작업을 취소합니다. 생성 후")
            self.navigator.cancelTask()
        ...
        while self.custom_plan_in_progress and len(self.semantic_maps) < self.semantic_map_input_dim:
            ...
            if distance_to_goal <= 1.0:
                global_goal_distance = self.calc_goal_distance(self.robot_pose, self.goal_pose)
                if global_goal_distance<=1.0:
                    self.get_logger().info("현 위치가 글로벌 목적지 부근입니다. 주행을 계속합니다.")
                    if global_goal_distance <= 0.5:
                        self.get_logger().info("글로벌 목적지 부근에 도달하였습니다.")
                        self.navigator.cancelTask()
                        self.record_first_state =True
                        _, _ = self.set_crop_center_position(self.robot_pose)
                        closed_status = True
    ...
    def get_dynamic_obs_pose(self, msg):
        ...
        # 만약 탐지된 객체가 있는 경우 
        if self.is_driving :
            # self.get_logger().info(f"탐지된 동적 객체 항목 : {dynamic_objects}")
            # 녹화중 아님, 동적 객체 탐지됨.
            self.get_logger().info(f"self.record_first_state ; {self.record_first_state}")
            if self.record_first_state:
                _, _ = self.set_crop_center_position(self.robot_pose)

                # self.get_logger().info(f"로봇의 시멘틱맵 중심점이 설정되었습니다. 현재로봇 위치 : {self.robot_pose}")
    ...
    def update_semantic_map(self):
        ...
        # if not self.custom_plan_in_progress and self.is_driving or not self.the_first_start_drivi ng:
        if not self.custom_plan_in_progress and self.is_driving:
            self.get_logger().info("fhi")
            # 만약 동적 객체가 범위 내에 있는 경우
            if self.dynamic_in_area_status:
                # self.the_first_start_driving = False
                self.semantic_map_save_count += 1
                self.undetectable_count_total = 0
    ...
    def set_crop_center_position(self, current_pose):
        ...
        if self.record_first_state:
            ...
            if self.the_first_start_driving:
                self.the_first_start_driving = False
                center_offset = 0
            else:
            ...
            # self.get_logger().info("시멘틱 맵을 발행하였음.")
            # 데이터 타입을 uint8로 변환
    ...
    def make_semantic_map(self, cropped_map, dynamic_objects, initial_path, laser_scan):
        ...
        if 0 <= current_robot_x_on_image < cropped_map.shape[1] and 0 <= current_robot_y_on_image < cropped_map.shape[0]:
            ...
            self.get_logger().info(f"로봇의 좌표가 크롭된 이미지 내에 포함되어 있지 않음. 현재 로봇 위치 : {self.robot_pose}")
        ...
        if self.record_first_state:
            ...
            if not goal_within_crop:
                ...
                self.get_logger().info(f"x min :{self.x_min}")
                self.get_logger().info(f"y min :{self.y_min}")
                ...
                for i in range(len(cropped_path) - 1):
                    ...
                    elif (0 <= x1 < cropped_map.shape[1] or 0 <= y1 < cropped_map.shape[0]) or \
                        (0 <= x2 < cropped_map.shape[1] or 0 <= y2 < cropped_map.shape[0]):
                            # 두 점 중 하나라도 맵 경계 내에 있는 경우 보라색 점으로 표시
                            # self.get_logger().info(f"좌표 ({x1}, {y1}) 또는 ({x2}, {y2}) 중 하나가 범위 {cropped_map.shape[1]} , {cropped_map.shape[0]}내에 포함되어 있음.")
                            cv2.circle(semantic_cropped_map, (x1, y1), 1, (255, 0, 255), -1)  # 보라색 점 그리기
                            cv2.circle(semantic_cropped_map, (x2, y2), 1, (255, 0, 255), -1)  # 보라색 점 그리기
...
seed = 34.0 #31 까지
test_approach = 'redip_not_astar_no_dynamic_no_local_goal_crowd'
