...
class TestDynamicObsAvoidance(Node):
    ...
    def __init__(self):
        ...
        self.semantic_map_timer = self.create_timer(0.2, 
        ...
        self.timer = self.create_timer(30.0, self.set_goal_and_start_navigation, callback_group=nav2_cb_group)
        ...
        # 저장할 시멘틱 맵 리스트
        self.semantic_map_input_dim = 5 # 이미지 입력 수는 10개로 지정.
        self.semantic_maps = []
        ...
        self.map_image = None 
        # self.map_dir = "/home/kmg/nav2_swerve/src/swerve_drive_navigation/map/just_forward"
        # self.map_dir = "/home/kmg/nav2_swerve/src/swerve_drive_navigation/map/just_forward_with_static"
        self.map_dir = "/home/kmg/nav2_swerve/src/swerve_drive_navigation/map/U_shape_map/map.yaml"
        self.load_map()
    ...
    def initialize_diffuison_model(self):
        #load models
        model_save_path = '/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/trained_model_goalcond_20eq5img_t50_ep300.pth'
        # model_save_path = '/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/trained_model_goalcond_20eq5img_ep200.pth'
        # parameters
# 
        # self.num_diffusion_iters = 75

        self.num_diffusion_iters = 40       
        ...
        # if use_tensor_rt:
        #     vision_encoder_tensorrt = TRTModule()
        #     vision_encoder_tensorrt.load_state_dict(torch.load('/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/tensorrt/vision_tensorrt_encoder.pth'))
        
        #     self.test_nets = nn.ModuleDict({
        #         'vision_encoder': vision_encoder_tensorrt,
        #         'noise_pred_net': self.test_nets_before_tensorrt['noise_pred_net']
        #     })
        # else:
        ...
        self.get_logger().info(f"TensorRT 사용 여부 :{use_tensor_rt}")
    ...
    def inference_path(self, nagent_poses, nimages, ngoal):
        ...
        with torch.no_grad():
            ...
            # self.get_logger().info(f"nimages : {nimages.size()}")
            # self.get_logger().info(f"nagent_poses : {nagent_poses.size()}")
            # self.get_logger().info(f"ngoal : {ngoal.size()}")
            ...
            # self.get_logger().info(f"naction 액션이 input값 : {naction.size()}")
            # self.get_logger().info(f"위치, 행동, 목적지가 컨디션 : {obs_cond.size()}")
    ...
    def load_map(self):
        ...
        # PGM 파일 경로 추출
        pgm_file_path = os.path.join(os.path.dirname(self.map_dir), map_data['image'])

        # PGM 이미지 로드
        self.map_image = cv2.imread(pgm_file_path, cv2.IMREAD_UNCHANGED)

        # PGM 파일을 읽어서 Numpy 배열로 변환
        self.map_image = np.array(self.map_image)

        # 필요한 경우 이미지 데이터 변환
        if map_data['negate']:
            self.map_image = cv2.bitwise_not(self.map_image)
        ...
        self.costmap_np =  self.initialize_map_image(self.map_image)

        output_image_path = "/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/map_data/원본.png"
        cv2.imwrite(output_image_path, self.map_image)

        output_image_path = "/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/map_data/costmap.png"
        cv2.imwrite(output_image_path, self.costmap_np)
    ...
    #         self.width = self.costmap.info.width
    #         self.height = self.costmap.info.height
    #         self.resolution = self.costmap.info.resolution
    #         self.origin_x = self.costmap.info.origin.position.x
    #         self.origin_y = self.costmap.info.origin.position.y
    ...
    def initialize_map_image(self, original_map):
        # Costmap 데이터를 2차원 배열로 변환
        costmap_data = np.array(original_map).reshape(self.height, self.width)
    ...
    def laser_callback(self,msg):
        ...
        """
        self.laser_scan = msg
        min_distance = 0.2  # 이 거리 이내에 장애물이 있을 경우 충돌 상태로 간주
        sensor_min_range = min(msg.ranges)
        # 라이다 센서의 최소 거리 값을 확인
        if  sensor_min_range >= 0.1 and  sensor_min_range <= min_distance and self.is_driving:
    ...
    def goal_callback(self, msg):
        ...
        self.get_logger().info(f"목적지 : {self.goal_pose.pose.position.x}, {self.goal_pose.pose.position.x}")
        # 새로운 목표 지점으로 이동 시작
        ...
        while True:
            ...
            if self.check_special_condition():
                ...
                goal_distance = self.trigger_custom_plan()
                ...
                if goal_distance > 0.45:
                    ...
                    self.get_logger().info(f"목적지 부근 도착으로 주행을 종료합니다.")
                    self.is_driving = False
                    break
    ...
    def trigger_custom_plan(self):
        ...
        custom_path, only_poses = self.create_custom_path()
        self.during_create_custom_path = False
        ...
        # followWaypoints()를 쓸지 follow_path()를 쓸지 고민해야 할듯
        ...
        closed_status = False
        # self.get_logger().info(f"해당 경로가 끝을 지을 수 있는지? {closed_status}, 경로와 목적지간 최소 거리 : {min_dist}")
    ...
    def get_back_original_path(self,path):
        ...
        for point in path:
            ...
            cv2.circle(self.accumulate_semantic_map, (int(unnorm_x), int(unnorm_y)), 1, (0, 0, 255), -1)
            # resize 되었던 값 다시 돌리기
    ...
    def get_dynamic_obs_pose(self, msg):
        ...
        # 만약 탐지된 객체가 있는 경우 
        if self.is_driving :
            # self.get_logger().info(f"객체 정보 받음:{dynamic_objects}")
            # self.get_logger().info(f"녹화가 처음인가요 ? :{self.record_first_state}")
            # 녹화중 아님, 동적 객체 탐지됨.
            if dynamic_objects: 
                if self.record_first_state:
                    center_x, center_y = self.set_crop_center_position(self.robot_pose)
                    # self.get_logger().info(f"crop_x:{center_x}, crop_y:{center_y}")
    ...
    def set_crop_center_position(self, current_pose):
        if self.record_first_state:
            ...
            # # 로봇의 회전 상태를 반영하여 yaw 값 계산 -> 크롭시 로봇의 중점과 현재 방향을 파악하기 위함.
            # orientation_q = [
            ...
            # 맵 데이터를 numpy 배열로 변환
            map_data = cv2.cvtColor(self.costmap_np, cv2.COLOR_BGR2GRAY)
            
            # 크롭된 맵 데이터 가져오기 (맵 경계를 넘는 영역은 검은색으로 설정)
            cropped_map_data = np.full((self.crop_size, self.crop_size), 100, dtype=np.int8)  # 기본값은 검은색 (장애물)
            ...
            x_min_crop = max(0, self.half_crop_size - (crop_x_in_img - x_min_map))
            y_min_crop = max(0, self.half_crop_size - (crop_y_in_img - y_min_map))

            cropped_map_data[y_min_crop:y_min_crop + (y_max_map - y_min_map), x_min_crop:x_min_crop + (x_max_map - x_min_map)] = map_data[y_min_map:y_max_map, x_min_map:x_max_map]
            
            # 맵 이미지 변환 (initialize_map_image)
            map_image = np.zeros((cropped_map_data.shape[0], cropped_map_data.shape[1], 3), dtype=np.uint8)

            for y in range(cropped_map_data.shape[0]):
                for x in range(cropped_map_data.shape[1]):
                    if cropped_map_data[y, x] == -1:
                        # map_image[y, x] = [128, 128, 128]  # 회색 (알 수 없음)
                        map_image[y, x] = [255, 255, 255]  # 흰색 (빈 공간)
                    elif cropped_map_data[y, x] == 0:
                        map_image[y, x] = [0, 0, 0]  # 검은색 (장애물)
                    else:
                        map_image[y, x] = [255, 255, 255]  # 흰색 (빈 공간)
    ...
    def make_semantic_map(self, cropped_map, dynamic_objects, initial_path, laser_scan):
        ...
        # self.get_logger().info(f"semantic map 생성")
        
        # center도 resolution을 고려한 즉, 계산한 값이여야함
        ...
        # 로봇의 현재 yaw 각도 계산 ->라이다 각도 정보 보정을 위함.
        ...
        if dynamic_objects:
            ...
            # self.get_logger().info(f"동적 객체가 어디있을까요~: ({dynamic_objects})")
            for obj in dynamic_objects:
                ...
                cv2.circle(self.accumulate_semantic_map, (obj_x, obj_y), 5, (255, 255, 0), -1)
                cv2.circle(semantic_cropped_map, (obj_x, obj_y), 5, (255, 255, 0), -1)
        ...
        semantic_map = self.save_img_correct_side(self.accumulate_semantic_map)
        ...
        cv2.imwrite(f"/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/map_data/semantic_map_total.png", semantic_map)
        cv2.imwrite(f"/home/kmg/nav2_swerve/src/swerve_drive_semantic_navigation/map_data/semantic_map_{self.semantic_map_save_count}.png", semantic_cropped_map)
        
        return semantic_map, semantic_cropped_map, robot_position_image
